...
- `data-brand-color` 使用企业品牌色，仅接受 `#RGB` 或 `#RRGGBB`，非法值回退默认蓝色。
...
- iframe 初始标题使用“在线咨询”，企业公开配置加载成功后更新为“企业简称 在线咨询”。
...
v0.9.1 对 AI 自动沉淀的 high 意向和人工转接线索增加轻量去重：当咨询提供了可识别的相同电话、邮箱或微信等联系方式，并且最近 24 小时内已有对应线索时，系统更新原线索的最新需求、备注、意向等级和人工跟进原因，不再重复创建新记录。没有有效联系方式时仍沿用原有创建逻辑。

该能力是 Demo 级规则，仅按规范化联系方式和时间窗口处理，不进行客户身份识别、模糊相似度判断或 CRM 主数据合并。
...
v0.9.1：Widget 品牌色、企业标题与 24 小时线索轻量去重
```
...
- 线索去重仅按联系方式和 24 小时时间窗口处理，不等同于 CRM 主数据合并。
